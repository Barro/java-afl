...
for mode in Forking Deferred Persistent; do
    rm -rf out/fuzz-"$mode"
    timeout --preserve-status -s INT "$test_timeout" ./java-afl-fuzz -m 30000 -i in/ -o out/fuzz-"$mode" -- java -cp out/ins test."$mode"
...
rm -rf out/fuzz-Null
timeout --preserve-status -s INT "$test_timeout" ./java-afl-fuzz -m 30000 -i in/ -o out/fuzz-Null -- java -cp out/ins test.Null
...
timeout --preserve-status -s INT "$test_timeout" ./java-afl-fuzz -m 30000 -i in/ -o out/fuzz-Crashing -- java -cp out/ins test.Crashing
